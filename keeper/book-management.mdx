...
description: How the keeper shapes, pins, refills, and trims the native bin books.
---
...
2. **Check the drift gate.** The book only re-pins when `|NAV - base| >=` the per-market gate - 20bps for indices/metals, 25–30bps for single names. The gates were swept empirically: the optimum sits just below each market's toxic-arbitrage line (first funded bin plus the paired-arb fee hurdle). Tighter gates burn transactions repositioning a book nobody can exploit; looser gates donate depth.
