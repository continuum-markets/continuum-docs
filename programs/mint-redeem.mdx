...
Cross-market collateral move: transfer cUSDC from an over-collateralized market's vault directly to an under-collateralized one's, funding it faster than waiting for fresh capital. The **source floor** is enforced on-chain: the source market must remain at or above its *own* dynamic target after the move, only liquid (non-yield-deployed) collateral moves, and the source's TWAP must be fresh. Both markets must share the keeper authority.
