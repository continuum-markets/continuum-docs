...
| [`clp`](/programs/clp) | `8xauDRjw9XRyk4FE3hW1JKjD8nC87gfr59Xig1dJqLES` | Continuous Liquidity Provider - vaults, native bin books, committed orders, instant swap. |
...
clp ─── hosts ──► native bin books + committed orders + instant swap
...
For trading L/S, the venues are native: the per-side bin books for instant size, committed orders for anything larger - see [Trade](/flows/trade).
