...
It is not a perp DEX, not a margin platform, not a wrapped-asset protocol. There is no funding rate, no liquidations, no leverage. The position **is** an SPL token. (The short leg does carry a [volatility decay](/concepts/nav#volatility-decay) in place of funding — driven by realized volatility, not by positioning. Longs carry no holding cost.)
...
- **No funding rate.** The constant-product NAV invariant bounds pair value naturally; no one needs to drain holders of the popular side. The short leg instead carries a [volatility decay](/concepts/nav#volatility-decay) that charges realized volatility rather than positioning, and never touches longs.
