...
```
   ...
   │       mint-redeem        │   │              clp                │
   │                          │   │                                 │
   │  mint_paired             │   │  1. bin books     (pool_swap)  │
...
|---|---|---|---|
| **bin book** | `pool_swap` | Instant, walks on-chain bins with real slippage | Small trades, immediate execution |
...
### 1. bin books - instant, small

Each market has one protocol-native pool **per side** (16 pools live: 8 markets × L/S). Bins live inline in the `Pool` account with a 2-zone geometry: fine core steps near NAV, wide wings further out (QQQ: 20 bps core, 300 bps wings). The keeper re-pins the book base to NAV when drift exceeds a per-market gate (20 bps for the indices/metals, wider for higher-vol single names).
