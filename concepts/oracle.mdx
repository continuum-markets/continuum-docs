...
The `Normal`-mode on-chain Pyth read (the mainnet pricing path, used by every mint/redeem when `risk_state == Normal`) enforces its own freshness: it rejects a print older than `MAX_PYTH_AGE_SECS` (60s, via `get_price_no_older_than`) and one whose confidence half-width exceeds `MAX_PYTH_CONF_BPS` (2% of price), returning `OraclePriceUnavailable`. A halted or wide feed therefore fails closed instead of pricing against a stale mark. (Devnet never exercises this path — it runs `ProxyMode`, below.)
...
- **The bin books keep trading** inside their standing band - off-hours price discovery within the last posted spread.
