---
title: Solvency
description: The paired-neutrality invariant, the one attack that exists against it, and the layered defenses - depth caps, vol-scaled fees, dynamic buffer, volatility decay.
---
...
```
S_NAV = (K / P) × γ              γ ratchets down by realized per-cycle variance
```

This **banks the choppiness-convexity the protocol bleeds as collateral surplus**. The r39 calibration shows the right decay (`Δγ/γ = σ²` per cycle) banks *exactly* the short-leg gamma on a choppy path. Crucially it charges **realized variance, not direction**: a smooth directional run barely moves γ (that payoff is in the untouchable long leg), so decay is the lever for the *variance* regime, while a directional single-name run still leans on the buffer, OI caps, and asset selection. **Decay only charges realized variance (choppiness), not directional single-name drift** — so single-name solvency rests on the two-pool bin book + solvency buffer + OI cap, not on γ. Ratchet-down only, floored at γ = 0.5, off-chain gated by `CLMM_GAMMA_DECAY_ENABLED`, which **defaults ON** (solvency-critical; the kill switch is `CLMM_GAMMA_DECAY_ENABLED=false`). It is **not** a perp funding rate and **never touches longs**. See [NAV → Volatility decay](/concepts/nav#volatility-decay).
