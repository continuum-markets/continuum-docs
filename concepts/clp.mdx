...
The **CLP** (Capital and Liquidity Provider) is the on-chain program where all trading happens. It hosts the three venues - native CLMM books, committed orders, and the instant oracle swap - plus the per-market vaults that stock them with inventory.
...
<Note>
  In v1 the CLP was a proxy for external Meteora DLMM positions. Meteora is retired; the CLP now runs its own books on-chain.
</Note>
