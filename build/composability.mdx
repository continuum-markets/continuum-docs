...
Implementation: a TypeScript helper that builds both ixns in one transaction. Or, for atomicity, a small on-chain program that wraps the two ixns in a single instruction (CPI to mint-redeem + CPI to Meteora's swap).
