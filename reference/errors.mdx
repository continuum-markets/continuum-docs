...
| 6028 | `MarketNotEmpty` | `close_market_admin` on a market that still has outstanding L/S supply or collateral | Admin-only. Closing would brick redemptions of live tokens, so the market must be fully wound down (`total_l_supply == total_s_supply == total_collateral == deployed_to_yield == 0`) first. |
