...
**Constant-product invariant** - `L_NAV × S_NAV = initial_l × initial_s`. The relationship that bounds combined pair value and removes the need for positioning-driven funding. (Convexity is charged separately, by the short leg's volatility decay.)
...
**Drift gate (reposition gate)** - The per-market threshold (20bps for indices/metals, wider for single names) a book's drift from NAV must exceed before the keeper re-pins it. Swept empirically to sit just below each market's toxic-arbitrage line.
...
**Native book / bin book** - Continuum's own per-side quote book: one `Pool` account per (market, side), bins inline (max 16, no per-bin rent), filled by `pool_swap`. The instant-trade venue. It is a **discrete-bin book**, not a concentrated-liquidity AMM - see [What the book actually is](/concepts/clp#what-the-book-actually-is). The `CLMM_*` keeper env vars and the `clmm.rs` module keep the older label for compatibility.
