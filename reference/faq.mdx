...
No. Perp DEXs use funding rates, margin, and liquidations. Continuum has none of these - positions are pre-paid SPL tokens (the short leg carries a volatility decay in place of funding, charged on realized volatility rather than positioning), the long/short pair has a constant-product NAV invariant that bounds combined value, and there's no leverage. The position **is** an SPL token; you can transfer it, deposit it elsewhere, or redeem it.
...
That rules out *positioning-driven* funding: you are never charged for being on the crowded side. It does not mean holding is free on both legs. The short leg carries a [volatility decay](/concepts/nav#volatility-decay) (the γ index), which pays for the convexity a long/short pair is exposed to. It differs from a perp funding rate in that it tracks **realized volatility rather than positioning**, only ever moves one way (it can never credit a holder), and **never touches the long leg**.
